...
exports.getCustomerPurchaseAnalytics = getCustomerPurchaseAnalytics;
 ...
 */
async function getSubscriptionDetails(merchantId) {
    ...
    if (!subscription) {
        return {
            plan: "free",
            status: "active",
            features: exports.SUBSCRIPTION_PLANS.free.features,
            limits: exports.SUBSCRIPTION_PLANS.free.limits,
        };
    }
    const planInfo = exports.SUBSCRIPTION_PLANS[subscription.planName];
    return {
        ...subscription,
        features: planInfo.features,
        limits: planInfo.limits,
        daysUntilNextBilling: subscription.currentPeriodEnd ? Math.ceil((subscription.currentPeriodEnd.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : 0,
    };
}
/**
 * Check if user can perform action based on subscription limits
 */
async function checkSubscriptionLimit(merchantId, action) {
    const subscription = await subRepo().findOne({
        where: { merchantId },
        order: { createdAt: "DESC" },
    });
    ...
    const maxBusinesses = "maxBusinesses" in planInfo ? planInfo.maxBusinesses : -1;
    switch (action) {
        case "createBusiness":
            if (maxBusinesses !== -1) {
 ...
 */
async function getPaymentAnalytics(merchantId, period = "month") {
 ...
 */
async function getCustomerPurchaseAnalytics(customerId) {
 ...
 * Get billing history
 */
async function getBillingHistory(merchantId, limit = 12) {
    ...
    return subscriptions.map((sub) => {
        const planInfo = exports.SUBSCRIPTION_PLANS[sub.planName];
        return {
            id: sub.id,
            plan: sub.planName,
            amount: planInfo.price,
